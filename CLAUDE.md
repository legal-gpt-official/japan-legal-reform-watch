...
- Filter state is reflected in URL query parameters (`q`, `area`, `stage`, `source`, `impact`, `ai`). `source` uses compact slugs such as `jftc`, `moe`, `ppc`, and `egov`; filters apply to the full public dataset, Load more state is not persisted in the URL, and Reset clears both filters and URL query parameters.
