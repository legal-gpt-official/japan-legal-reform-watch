...
- **GitHub Actions daily update workflow exists** at `.github/workflows/daily-update.yml` for manual runs and a daily 21:00 UTC schedule (06:00 JST). It commits only `data/raw_items.json`, `data/summary_cache.json`, and `docs/data/legal_updates.json` when the published data file changes. The workflow first runs `py_compile` on all three pipeline scripts and the offline regression tests (`python -m unittest discover -s tests`); any failure stops the job **before** the network fetch, build, summarize, and commit steps. `ANTHROPIC_API_KEY` is exposed only to the summarize step, never to the tests.
