...
- Stage 1 **raw fetch only**: pulls a small `SOURCES` list of Japanese government / regulator feeds (RSS / RDF / Atom) into `data/raw_items.json`. Current sources include e-Gov Public Comment, FSA, METI, MHLW, Digital Agency, and CAA. It does **fetch → normalize → de-duplicate → log** and nothing else.
