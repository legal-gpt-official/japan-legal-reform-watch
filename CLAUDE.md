...
- **Source names display English-first** via the `SOURCE_DISPLAY_NAMES` map in `docs/app.js` (e.g. `Japan Fair Trade Commission (JFTC)`); the underlying `source_name` values in the published JSON and the filter option VALUES stay unchanged. When adding a source to `fetch_updates.py`, add its English display name to that map (a test enforces this).
