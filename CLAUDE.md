...
- The dashboard supports Sort by `Relevance`, `Published date`, and `Last checked`. Sort applies to the full filtered dataset before the 50-card render window. Filter/sort state is reflected in URL query parameters (`q`, `area`, `stage`, `source`, `impact`, `sort`, `ai`), where `sort` supports `relevance`, `published`, and `checked`; Load more state is not persisted in the URL, and Reset returns Sort to Relevance while clearing URL query parameters.
