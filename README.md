...
`scripts/fetch_updates.py` fetches a curated set of Japanese public-sector feeds and official update pages (the `SOURCES` list inside the script) and stores **raw, de-duplicated** items in [`data/raw_items.json`](data/raw_items.json). Official-source coverage: **e-Gov Public Comment, FSA, METI, MHLW, Digital Agency, CAA, PPC, JFTC, MOJ (法務省), MOE (環境省), MOF (財務省), and MIC (総務省)**. PPC, JFTC, and MOE use lightweight official HTML page parsing because stable RSS endpoints were not found; the others are RSS/RDF/Atom (the MIC feed is Shift_JIS, handled by both parser paths). NTA (国税庁) was investigated and deferred — no RSS, and its legacy news page is dominated by statistics/PDF notices. It performs fetching, normalization, de-duplication, and logging **only** — it does **not** summarize, call any LLM, or modify the published `docs/data/legal_updates.json`.
...
- **Shareable filter URLs**: filter state is reflected in query parameters (`q`, `area`, `stage`, `source`, `impact`, `ai`). `source` uses compact slugs such as `jftc`, `moe`, `ppc`, and `egov`; Load more state is not persisted, and Reset clears both filters and URL query parameters.
- **English-first source labels**: the Source filter and each card's source name display English-first labels (e.g. `Japan Fair Trade Commission (JFTC)`, `Ministry of the Environment (MOE)`) via a display-name map in `docs/app.js`. The underlying `source_name` values in the published JSON — and the official Japanese `source_url` links — are unchanged.
- **Free-text search** across English title and summary (also against the full dataset).
