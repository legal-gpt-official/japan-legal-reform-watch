...
The manual [`japanese-summary-backfill.yml`](.github/workflows/japanese-summary-backfill.yml) workflow fills the complete published corpus without translating English. It uses bounded parallel direct calls for missing records, checkpoints each successful round to the dispatched branch, rebuilds browser archives, and verifies that every published item has complete Japanese content plus independent Claude provenance. Cache hits are free, so interrupted runs resume without repeating completed items. It shares the same data-writer concurrency group.
