...
`scripts/fetch_updates.py` fetches a small set of Japanese public-sector feeds and official update pages (the `SOURCES` list inside the script) and stores **raw, de-duplicated** items in [`data/raw_items.json`](data/raw_items.json). Current sources include e-Gov Public Comment, FSA, METI, MHLW, Digital Agency, CAA, PPC, and JFTC; PPC and JFTC use lightweight official HTML page parsing because stable RSS endpoints were not found. It performs fetching, normalization, de-duplication, and logging **only** — it does **not** summarize, call any LLM, or modify the published `docs/data/legal_updates.json`.
...
- **Filters** by Area, Stage, Source, and Impact Level, plus Quick filters for Public Comment Open, AI Summary, Medium Impact, and Reset.
- **Free-text search** across English title and summary.
- **Status line** showing visible results, visible AI-summary count, and the latest `last_checked` date among currently visible updates.
