...
A second, manual-only workflow [`translation-backfill.yml`](.github/workflows/translation-backfill.yml) accumulates the zh-Hans corpus **translate-only** — it never fetches, runs Stage 2, summarizes, or touches source-health. It counts translations before/after, runs up to 30 direct `translate_updates.py` calls, and enforces a semantic integrity gate: neither artifact may shrink, and every published translation must match a valid current-hash cache entry. Direct calls avoid withholding a whole checkpoint behind a long-running Message Batch. Its disabled-by-default `omit_title_ja_reference` recovery input can translate only the canonical English fields when a specific output repeatedly copies Japanese kana from reference context; it does not alter the stored Japanese original or normal daily behavior. Cache-only stale/history entries are allowed because changed English source text invalidates publication without requiring destructive cache cleanup. It then regenerates deterministic yearly browser shards, checks `origin/main` has not advanced, and commits only translation-derived artifacts. Both workflows share `concurrency.group: japan-legal-reform-data-writer`.
