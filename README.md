...
`scripts/fetch_updates.py` fetches a small set of Japanese public-sector feeds (the `SOURCES` list inside the script) and stores **raw, de-duplicated** items in [`data/raw_items.json`](data/raw_items.json). Current RSS / Atom sources include e-Gov Public Comment, FSA, METI, MHLW, Digital Agency, and CAA. It performs fetching, normalization, de-duplication, and logging **only** — it does **not** summarize, call any LLM, or modify the published `docs/data/legal_updates.json`.
