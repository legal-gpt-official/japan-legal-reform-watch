...
- **Stage 3 script exists, but AI summaries are generated only after `scripts/summarize_updates.py` is run with `ANTHROPIC_API_KEY`.** Before Stage 3 has been run, `docs/data/legal_updates.json` may contain only fixed-template English and may not yet include `summary_source`. Normal runs can still target a relevance-ranked top-N pool, while explicit language-only modes maintain the full corpus. Successfully processed English items receive `summary_source: "claude"`; untouched or failed items are marked `summary_source: "rule_based"`. `area` / `stage` / `impact_level` and ranking remain keyword rules (a technical heuristic, not a legal judgement).
...
For resumable corpus maintenance, `--all-items --english-only` generates only missing/current-key English summaries and preserves every Japanese field, while `--all-items --japanese-only` does the inverse. Both support bounded direct-call parallelism, call-count limits, measured token reporting, and a USD cap.
...
A second, manual-only workflow [`translation-backfill.yml`](.github/workflows/translation-backfill.yml) accumulates the zh-Hans corpus **translate-only** — it never fetches, runs Stage 2, summarizes, or touches source-health. It counts translations before/after, runs up to 30 direct `translate_updates.py` calls with a measured estimated-cost cap (USD 0.50 by default), and enforces a semantic integrity gate: neither artifact may shrink, and every published translation must match a valid current-hash cache entry. Direct calls avoid withholding a whole checkpoint behind a long-running Message Batch. Successful responses report base input/output and prompt-cache token usage plus a conservative USD estimate; capped runs stop scheduling after the measured threshold, with at most one bounded-call overshoot. Its disabled-by-default `omit_title_ja_reference` recovery input can translate only the canonical English fields when a specific output repeatedly copies Japanese kana from reference context; it does not alter the stored Japanese original or normal daily behavior. Cache-only stale/history entries are allowed because changed English source text invalidates publication without requiring destructive cache cleanup. It then regenerates deterministic yearly browser shards, checks `origin/main` has not advanced, and commits only translation-derived artifacts. Both workflows share `concurrency.group: japan-legal-reform-data-writer`.

The manual [`japanese-summary-backfill.yml`](.github/workflows/japanese-summary-backfill.yml) workflow fills the complete published corpus without translating English. Each dispatch is one bounded checkpoint with explicit call-count, parallelism, and measured estimated-cost caps; its safe defaults are a 10-item, single-call canary capped at USD 0.50. Successful responses report input/output tokens and estimated USD cost before the checkpoint is committed. It rebuilds browser archives and verifies that every published item has complete Japanese content plus independent Claude provenance before the workflow can finish successfully. Cache hits are free, so interrupted runs resume without repeating completed items. The daily workflow separately maintains new or changed Japanese items with at most 30 direct Opus calls, parallelism four, and a measured USD 0.50 cap; a provider-wide credit/auth failure raises a warning and leaves the rest of the legal-data pipeline running. Both paths share the same data-writer concurrency group.

The manual [`english-summary-backfill.yml`](.github/workflows/english-summary-backfill.yml) workflow fills canonical English summaries in bounded, resumable checkpoints. It preserves Japanese fields byte-for-byte at the semantic-field level and, in the same checkpoint, refreshes every zh-Hans translation invalidated by changed English text. The selected bounded parallelism applies to both phases. Separate Opus-summary and Sonnet-translation cost caps prevent either phase from running unbounded; safe defaults are a 10-item single-call canary with USD 0.50 per phase. If a refreshed Chinese title alone is rejected because it copied Japanese kana, that workflow may retry the item once without the Japanese title reference; both calls still count against the same request and cost limits. A public checkpoint is committed only if English coverage grows, Japanese fields remain unchanged, and Chinese coverage does not shrink. If translation instead fails after successful English calls, only the new English cache responses are saved and the published corpus is restored unchanged, so the next funded run does not pay for those English results again. The daily workflow then maintains new or changed English items across the full corpus with at most 30 calls, parallelism four, and a measured USD 0.50 cap.

The translator classifies provider (Anthropic) errors and **fails fast** on a run-fatal one — `insufficient_credit`, `authentication_error`, or `permission_error` — stopping further API calls on the first occurrence (the rest become `provider_aborted_items` / `api_calls_avoided`, not 30 repeated failures). A timeout or network failure affecting an entire optional Message Batch is likewise reported once as `provider_status: unavailable`, rather than being misreported as many independent item failures with a healthy provider. Error bodies, request ids, API keys, and source/translation text are never logged. `--provider-failure-mode` chooses the policy: the **daily** workflow uses `warn` (a provider outage does not stop fetch/build/summarize/commit; it raises a `::warning::Translation provider unavailable: <type>` annotation and adds no translations), while the **backfill** workflow uses `fail` (a provider-wide outage exits 1 before any commit). Per-item rate-limit / transient / network errors remain non-fatal, and a translation of changed Japanese source is still dropped to English even during an outage. (Running out of credit is resolved on the Anthropic platform — Plans & Billing — not in this code.)
...
- **Data status summary** computed client-side from the public JSON: total updates, sources represented, locale-specific content coverage, open public comment count, Newly detected count for the last 7 days, and latest checked date. Coverage is labelled and counted as English AI summaries in English, Japanese-source AI summaries in Japanese, and complete Simplified-Chinese translations in Chinese, so translation is not presented as summarization. It does not claim complete coverage or successful checking of every source.
...
- **Status line** in the form `Showing X of Y matching updates · Z total updates · [localized coverage]: N · Last checked: D` — localized coverage and latest `last_checked` cover the full filtered set, not just the rendered cards. The coverage label follows the active language: English AI summaries, Japanese AI summaries generated directly from Japanese source metadata, or complete Simplified-Chinese translations.
...
The four-stage pipeline (`fetch_updates.py` → `build_public_data.py` → `summarize_updates.py` → `translate_updates.py`) plus deterministic yearly archive generation (`build_public_archives.py`) is live in the daily GitHub Actions workflow. Stage 3 AI summaries and Stage 4 `zh-Hans` translations are present in the published corpus, with English remaining canonical and per-field fallback for untranslated items. Daily English and Japanese maintenance each scan the complete corpus with at most 30 new direct Opus calls, parallelism four, and separate measured USD 0.50 caps. Simplified-Chinese maintenance likewise allows at most 30 direct calls, reports actual response token counters, and uses a conservative measured USD 0.50 cap. Cache hits are free, so unchanged output is not regenerated. The dashboard is publicly hosted on GitHub Pages from `/docs`, and a successful daily data commit explicitly requests a Pages build from the latest `main` revision.
...
- Full-corpus English and Japanese maintenance is cost-capped and cache-backed. Periodically review live outputs and measured run costs; do not regenerate unchanged cache hits.
